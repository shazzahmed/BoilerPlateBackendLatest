...
namespace Infrastructure.Services
{
    ...
    public class TenantService : ITenantService
    {
        ...
        /// <summary>
        /// Checks if tenant has reached their student limit
        /// </summary>
        public async Task<bool> HasReachedStudentLimitAsync()
        {
            var tenant = await GetCurrentTenantAsync();
            ...
            return tenant.CurrentStudentCount >= tenant.MaxStudents;
        }

        /// <summary>
        /// Checks if tenant has reached their staff limit
        /// </summary>
        public async Task<bool> HasReachedStaffLimitAsync()
        {
            var tenant = await GetCurrentTenantAsync();
            ...
            return tenant.CurrentStaffCount >= tenant.MaxStaff;
        }
        ...
        /// Call this after adding/removing students or staff
        /// </summary>
        public async Task UpdateResourceCountsAsync()
        {
            ...
            var studentCount = await _context.Set<Student>()
                .Where(s => s.TenantId == tenantId && s.IsActive && !s.IsDeleted)
                .CountAsync();

            // Count active staff
            var staffCount = await _context.Set<Staff>()
                .Where(s => s.TenantId == tenantId && s.IsActive && !s.IsDeleted)
                .CountAsync();

            // Update tenant
            tenant.CurrentStudentCount = studentCount;
            tenant.CurrentStaffCount = staffCount;
